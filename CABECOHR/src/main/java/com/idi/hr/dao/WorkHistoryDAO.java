...
public class WorkHistoryDAO extends JdbcDaoSupport {
	...
	private static final Logger log = Logger.getLogger(WorkHistoryDAO.class.getName());
	...
	@Autowired
	public WorkHistoryDAO(DataSource dataSource) {
	...
	/**
	 * Get WorkHistory from DB
	 * 
	 * @return List of WorkHistory
	 * @throws Exception
	 */
	public List<WorkHistory> getWorkHistorys() {

		String sql = hr.getProperty("GET_WORK_HISTORYS").toString();
		log.info("GET_WORK_HISTORYS query: " + sql);
		WorkHistoryMapper mapper = new WorkHistoryMapper();

		List<WorkHistory> list = jdbcTmpl.query(sql, mapper);
		return list;
